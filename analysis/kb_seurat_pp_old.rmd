...
params:
  ...
  run.sct:
      ...
      value: true
  run.jackstraw:
       label: "run.jackstraw -- Perform Jackstraw analysis"
       value: true    
  add.spikes.ercc: 
...
```{r loading splice seperated dataset, message=FALSE, warning=FALSE, eval=params$isvelo}
...
```
```{r setting up phenotable from csv, eval=params$meta_data != ""}
...
## Setting up the phenotable ##
# Make phenodata matrix from column names
...
```{r Running plate QC, eval=params$method == "plate"}
...
res <- knitr::knit_child(here::here("analysis", norm.rmd), quiet = TRUE, envir=environment())
cat(res, sep = '\n')
```

```{r, normalizing dataset (quant), warning=FALSE, results = 'asis', eval=!params$isvelo}
res <- knitr::knit_child(here::here("analysis", "analysis-sct-quant.Rmd"), quiet = TRUE, envir=environment())
cat(res, sep = '\n')
```
...
```{r running JackStraw, eval=params$run.jackstraw}
...
```{r JackStraw plot, eval=params$run.jackstraw}
