...
ifelse(add.qc.ERCC, filters[["filter_by_ercc"]] <- sce_filt$pct_counts_ERCC < ERCC_pct_max, NULL)
ifelse(add.qc.MT, filters[["filter_by_mt"]] <- sce_filt$pct_counts_MT < mt_pct_max, NULL)
...
if(add.qc.MT) {
  pdf(paste(resultsdir,"MT_before+aftercellsFiltering.pdf", sep="/"))
  par(mfrow=c(2,2))
...
if (add.qc.ERCC) {
  pdf(paste(resultsdir,"ERCC_before+aftercellsFiltering.pdf", sep="/"))
  par(mfrow=c(2,2))
...
sce_usmatch <- calculateQCMetrics(
  sce_usmatch, feature_controls = control_features_us
  )
