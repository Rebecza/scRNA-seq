...
source(paste0(workdir,"/src/plate_qc.R"))
...
resultsdir <- paste(workdir,"/output/", "/R_analysis/", dateoftoday, result_descript, sep = "")
...
if (extract_phenotypes) {
  source(paste0(workdir,"/src/get_pheno_data.R"))
  phenodata <- get_pheno_data(data.df = data.df, combined_variables_to_id = combined_variables_to_id, plate_variables= plate_variables)
  identical(rownames(phenodata), colnames(spliced.data.df))
  write.csv(phenodata, paste(resultsdir,"phenodata_kbordered.csv", sep="/"))
} else {
  phenodata <- NULL
...
if (run.plate_qc) {
  ...
  out.file <- paste(resultsdir,"PlateDiag_lndscp.pdf",sep="/")
  plate_qc(data.df = data.df,barcode_file = barcode_file, spliced.data.df = spliced.data.df, out.file = out.file )
...
sce <- SingleCellExperiment(assays = list(counts = count_matrix), rowData = rownames(count_matrix))
if (!is.null(phenodata)) { 
  colData(sce) -> phenodata
}
```
...
if (add.qc.MT && add.qc.ERCC) {
  ...
  Nan_cells <- c(colnames(sce_filt)[sce_filt$pct_counts_MT == "ERCC"])
}
...
filters <- list()
filters[["filter_by_expr_features"]] <- sce_filt$total_features_by_counts >= total_feat_tresh
filters[["filter_by_total_counts"]] <- sce_filt$total_counts >= total_counts_tresh
if (add.qc.ERCC) {
  filters[["filter_by_ercc"]] <- sce_filt$pct_counts_ERCC < ERCC_pct_max
  table(filter_by_ercc)
}
if (add.qc.MT) {
  filters[["filter_by_mt"]] <- sce_filt$pct_counts_MT < mt_pct_max
  table(filter_by_mt)

}
#Combine filtered logis 
sce_filt$use <- Reduce("&", filters)
...
pdf(paste(resultsdir,"Histograms_before+aftercellsFiltering.pdf",sep="/"))
par(mfrow=c(2,2))
hist(sce_filt$total_counts, breaks = 100)
abline(v = total_counts_tresh, col = "red")
...
pdf(paste(resultsdir,paste0(result_descript,"MT+ERCC_before+aftercellsFiltering.pdf"), sep="/"))
par(mfrow=c(2,2))
...
saveRDS(seuset, paste(resultsdir,"seuset_qc+norm.rds",sep="/"))
```
...
pdf(paste(resultsdir,"Histograms_before+aftercellsFiltering_UnsplicedReads.pdf",sep="/"))
par(mfrow=c(2,2))
hist(sce_us$total_counts, breaks = 100)
abline(v = total_counts_tresh, col = "red")
hist(sce_us$total_features_by_counts, breaks = 100)
...
pdf(paste(resultsdir,paste0("VizPCAplot_PCs1-", pcs_max, ".pdf"), sep="/"), width = 20, height = 60)
...
pdf(paste(resultsdir,paste0("PCheatmap_PCs1-", pcs_max, ".pdf"), sep="/"), width = 20, height = 60)
DimHeatmap(
    object = seuset, 
    dims = 1:pcs_max, 
    cells = 500, 
...
pdf(paste(resultsdir, paste0("UMAPdiffsettings_lineages_", paste(as.character(pcs_for_overview), collapse = "-"),".pdf"),sep="/"), width = 20, height = 15)
...
saveRDS(seuset, file= paste(resultsdir,"seusetv3_scnormHVG_velocity.rds", sep="/"))
```
