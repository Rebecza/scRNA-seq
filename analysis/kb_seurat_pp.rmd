...
params:
  ...
  meta_type:
    label: "meta_type -- type of meta data (default samples)"
    choices: ["sample", 'cell']  
    value: "sample"
...
here::set_here()
knitr::opts_knit$set(root.dir = here::here(), output.dir = here::here())
#knitr::opts_knit$set(output.dir=here::here("analysis"))
...
```{r setting up phenotable from csv - sample, eval=params$meta_data != "" && params$meta_type == "sample"}
phenodata <- read_meta_basic(path=params$kb.dir, cell.names = colnames(data.df))


phenodata <- read_meta_sample(path = "~/Documents/Projects/R/scRNA-seq/data/sample_meta.csv",
                                cell.names = colnames(data.df),
                                group_id = c("library","timepoint"))
...
print("Setup meta-data from .csv file")
```

```{r setting up phenotable from csv - cells, eval=params$meta_data != "" && params$meta_type == "cell"}
## Load phenodata from meta data file from .csv file
phenodata_all <- read_meta_cells(path = "~/Documents/Projects/R/scRNA-seq/data/phenodata_kbordered.csv",
                                cell.names = colnames(data.df), group_id = "Library")
...
```{r setting up phenotable from filenames, eval=params$extract_meta_columns != ""}
# Make meta data table from column names
...
print("Derived phenodata from file names")
...
res <- knitr::knit_child(here::here(norm.rmd), quiet = TRUE, envir=environment())
cat(res, sep = '\n')
```

```{r, normalizing dataset (quant), warning=FALSE, results = 'asis', eval=!params$isvelo}
res <- knitr::knit_child(here::here("analysis-sct-quant.Rmd"), quiet = TRUE, envir=environment())
cat(res, sep = '\n')
```
