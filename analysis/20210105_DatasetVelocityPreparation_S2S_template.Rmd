...
output:
  ...
  html_document:
    df_print: paged
params:
  rmd.root : "analysis/20210105_DatasetVelocityPreparation_S2S_template.Rmd"
...
---
    ...
    ## Location of scripts ##
    source(here::here("./src/read_kb_dataset_s2s.R"))
    source(here::here("./src/plate_qc.R"))
    source(here::here("./src/get_pheno_data.R"))
...
if (!is.null(params$vars_to_regress)){
result_descript <- paste(params$result_descript, "regressed", sep = "_")  
}

dateoftoday <- gsub("-", "", as.character(Sys.Date()))
resultsdir <- paste(here::here(),"/output/", "/R_analysis/", dateoftoday, params$result_descript, sep = "")
...
pdf(paste(resultsdir,paste0("VizPCAplot_PCs1-", params$pcs_max, ".pdf"), sep="/"), width = 20, height = 60)
...
pdf(paste(resultsdir,paste0("PCheatmap_PCs1-", params$pcs_max, ".pdf"), sep="/"), width = 20, height = 60)
DimHeatmap(
    object = seuset, 
...
pdf(paste(resultsdir, paste0("UMAPdiffsettings_lineages_", paste(as.character(params$pcs_for_overview), collapse = "-"),".pdf"),sep="/"), width = 20, height = 15)
