...
params:
  kb.dir: 
    label: "kb-python sample directory"
    value: /Users/tilman_work/Documents/Projects/R/testset/kallistobus
  barcode_file: 
    label: "tab delimtied barcode file with plate ids (col 1) and barcodes (col 2)"
    value: /Users/tilman_work/Documents/Projects/R/scRNA-seq/data/barcode_384.tab
  mt_genes_file:
    label: "txt file with mitchondrial gene names"
    value: /Users/tilman_work/Documents/Projects/R/scRNA-seq/data/MT_genes.txt
  meta_data:
    label: "Meta data csv file"
    value: /Users/tilman_work/Documents/Projects/R/scRNA-seq/data/pbmc_meta_test.csv  #used when extract_phenotypes = FALSE
  resultsdir:
     label: "Output directory for qc files" 
     value: /Users/tilman_work/Documents/Projects/R/scRNA-seq/output
  plate_variables: 
      label: "Meta data variables to extract from sample names (if present)"
      value: Genome,Method,Lineage,Timepoint,Replicate,Library,Well 
  combined_variables_to_id: 
      label: "Meta data variables to add combined id column" 
      value: Method,Lineage,Timepoint,Library
  umap_cols: 
      label: "Meta data variables for Seurat's RunUMAP. Can be seperated by comma for multiple values"
      value: Method,Timepoint  
  confounders_to_test: 
      label: "Meta data variabel for confounder testing"
      value: Library
  lab_col: 
      label: "Library meta data column "
      value: Library
  remove_bc: 
      label: "Replace barcode id with plate id in sample names. Enable for inhouse-celseq"
      value: true 
  run.plate_qc: 
      label: "Run plate qc. Enable for inhouse-celseq"
      value: true
  extract_phenotypes: 
      label: "Extract meta data information from filenames. Uses columns defined in plate_variables" 
      value: true
  add.spikes.ercc: 
      label: "Perform ercc spike-ins qc (if present)"
      value: true
  ercc_pct_max: 
      label: "max percentage ercc spike-ins"
      value: 20
  add.spikes.mt: 
      label: "Perform mitochondrial qc"
      value: true
  mt_pct_max: 
      label: "max percentage mitochondrial genes"
      value: 50
  amount_cells_expr: 
      label: "threshold for amount of cells considered expressed"
      value: 0
  total_counts_tresh: 
      label: "threshold for total UMI counts"
      value: 1000
  total_feat_tresh: 
      label: "threshold for total number of feature (genes) per proportion of cells."
      value: 500
  gene_tresh: 
      label: "threshold for total number of genes after qc"
      value: 0
  nhvg: 
      label: "threshold for number of variables features (genes)"
      value: 2000
  explore_violin: 
      label: "violin expression plot for genes (can be seperated by comma)."
      value: SOX2,GAPDH
  filtering: 
      label: "perform filtering" 
      choices: ['in', 'out', 'none']
      value: 'none'
  new_col_pattern:
      label: "new column pattern"
      value:  _S
  old_col_pattern: 
      label: "old column pattern"
      value: ""
  pcs_for_overview:
      label: "principal components for overview in combined umap plot"
      value: '10,20,30,40,50'
  pcs_max: 
      label: "max number of principal components to visualize"
      value: 70
  subset_id: 
      label: "subset ids"
      value: ''
  vars_to_regress:
      label: "variables to perform regression analysis"
...
source(here::here("../src/read_kb_dataset_s2s.R"),local = knitr::knit_global())
source(here::here("../src/plate_QC.R"), local = knitr::knit_global())
source(here::here("../src/get_pheno_data.R"), local = knitr::knit_global())
...
```{r loading dataset, echo = TRUE, warning = FALSE, linewidth=60}
...
```{r filtering certain entries, linewidth=60}
...
length(colnames(spliced.data.df))/n_bc
```


```{r phenotable, linewidth=60}
## Setting up the phenotable ##
if (params$extract_phenotypes) {
  phenodata_all <- get_pheno_data(data.df = data.df,
  ...
  phenodata_all <- read_pheno_data(path = params$meta_data,
                               data.df = data.df,
                               combined_variables_to_id = combined_variables_to_id,
...
```{r params$filtering empty entries, echo = FALSE, linewidth=60}
...
if (params$add.spikes.ercc) {
  isSpike(sce, "ERCC") <- grepl("^ERCC2-", rownames(sce))
...
NaN_cells <- NULL
if (params$add.spikes.mt && params$add.spikes.ercc) {
  NaN_cells <- unique(c(colnames(sce)[sce$pct_counts_ERCC == "NaN"],
                        colnames(sce)[sce$pct_counts_MT == "NaN"]))
} else if (params$add.spikes.mt) {
  NaN_cells <- c(colnames(sce)[sce$pct_counts_MT == "NaN"])
} else if (params$add.spikes.ercc) {
  NaN_cells <- c(colnames(sce)[sce$pct_counts_ERCC == "Nan"])
}
if (!is.null(NaN_cells)) {
...
plot.list = list(
  p1 = plotColData(sce, y = "total_counts", x = "Library"),
  p2 = plotColData(sce, y = "total_features_by_counts", x = "Library")
  )
...
```{r filtered dataset: compare before/after filtering, linewidth=60}
...
```{r endogenous dataset for confounding factors,  linewidth=60}
#------------------------------
...
```{r PCA on raw data,  linewidth=60}
...
```{r raw log2-transformation,  linewidth=60}
...
```{r create seurat object, linewidth=60}
seuset <-
  CreateSeuratObject(
  counts = counts(sce_endo),
  assay = "sf",
  meta.data = as.data.frame(colData(sce_endo)[, 1:(length(plate_variables) +
  1)])
  )
```
...
```{r seurat objects to sce,  linewidth=60}
...
```{r check confounders in raw dataset,  linewidth=60}
...
pdf(paste(
  params$resultsdir,
  paste0("VizPCAplot_PCs1-", params$pcs_max, ".pdf"),
  sep = "/"
  ),
  width = 20,
  height = 60)
...
pdf(paste(
  params$resultsdir,
  paste0("PCheatmap_PCs1-", params$pcs_max, ".pdf"),
  sep = "/"
  ),
  width = 20,
  height = 60)
...
```{r running JackStraw, linewidth=60}
