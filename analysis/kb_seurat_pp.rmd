...
params:
  ...
  barcode_file: 
    label: "tab delimtied file with barcode sequences. For plate based method, use two column layout"
  ...
  meta_data:
    label: "Meta data csv file. Used when extract_meta_columns is not set"
  ...
  meta_group_id: 
      label: "Meta data column grouping for visualization. Adds an extra column to meta data" 
      value: Library
  umap_cols: 
  ...
  nhvg: 
      ...
      value: 2000
  cell_id_subset: 
      label: "cell ids to include/exclude based on cell_id_subset_filter"
      value: ''    
  cell_id_subset_filter: 
...
source("../src/read_kb_dataset_s2s.R")
source("../src/plate_QC.R")
source("../src/get_pheno_data.R")
...
if (params$cell_id_subset_filter == "out"){
  ...
  length(colnames(data.df[,!grepl(params$cell_id_subset, colnames(data.df)) == TRUE]))
  # filter cells based on the substring.
  data.df <- data.df[,!grepl(params$cell_id_subset, colnames(data.df)) == TRUE]
} else if (params$cell_id_subset_filter == "in"){
  # checking the amount of cells in the dataset before filtering
  length(colnames(data.df))
  # the amount of cells you retrieve with the filter.
  length(colnames(data.df[,grepl(params$cell_id_subset, colnames(data.df)) == TRUE]))
...
if (params$extract_meta_columns != "") {
  phenodata_all <- get_pheno_data(data.df = data.df,
                                  combined_variables_to_id = meta_group_id, 
                                  plate_variables= extract_meta_columns)
