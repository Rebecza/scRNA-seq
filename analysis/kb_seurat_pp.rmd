...
```{r Perform SCTransform, warning=FALSE}
 ##Perform SCTransform
sct <- SCTransform(seuset, assay = "sf", vars.to.regress = params$vars_to_regress_sf, verbose = FALSE)
...
# Here we take te logp1 normalized counts from the sct object (data slot)
filtered_cells <- intersect(rownames(phenodata), 
                            colnames(sct@assays$SCT@data))
pheno_matchedseuset <- phenodata[filtered_cells, ]
pheno_orderedseuset <-
pheno_matchedseuset[match(colnames(sct@assays$SCT@data),
...
sct_uf <- SCTransform(seuset, assay = "uf",vars.to.regress = params$vars_to_regress_uf, verbose = FALSE)
```
