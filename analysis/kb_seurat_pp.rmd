...
params:
  ...
  meta_data:
    ...
    value: ~/Documents/Projects/R/scRNA-seq/data/examples/cell_meta_example.csv
  extract_meta_columns: 
    ...
    value: ""  #genome,spike-in,reporter,cell_type,timepoint,library,index,combined_id
  resultsdir:
...
all_samples <- list.files(params$kb.dir, recursive = FALSE, include.dirs = FALSE)
phenodata <- read_meta_basic(sample_folders=all_samples, cell.names = colnames(data.df))
pheno_len <- 1
pheno_matched <-  phenodata

if (params$meta_data != "" && params$extract_meta_columns == "") {
